...
#[tokio::test]
async fn create_wall_post_with_text_line_creates_quote_entry() {
    let (_, app, _, work_id) = setup_work_uploaded().await;
    ...
    let wall_post_id: Uuid = sqlx::query_scalar!(
        r#"SELECT id FROM wall_posts WHERE work_id=$1"#,
        work_id
    )
    ...
    let count: i64 = sqlx::query_scalar(
        r#"SELECT COUNT(*) FROM work_quotes WHERE wall_post_id=$1"#,
    )
    .bind(wall_post_id)
    .fetch_one(&app.state.db_pool)
    .await
...
async fn create_wall_post_without_text_line_creates_pin_entry() {
    ...
    let count: i64 = sqlx::query_scalar(
        r#"SELECT COUNT(*) FROM work_pins WHERE wall_post_id=$1"#,
    )
    .bind(wall_post_id)
    .fetch_one(&app.state.db_pool)
    .await
...
async fn create_wall_post_sets_timestamp() {
    ...
    assert!(created_at.created_at.is_some());
}
...
#[tokio::test]
async fn create_wall_post_with_empty_text_line_creates_pin() {
    let (_, app, _, work_id) = setup_work_uploaded().await;
    ...
    let quote_count: i64 = sqlx::query_scalar(
        r#"SELECT COUNT(*) FROM work_quotes WHERE wall_post_id=$1"#,
    )
    .bind(wall_post_id)
    .fetch_one(&app.state.db_pool)
