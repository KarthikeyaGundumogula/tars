...
impl Resource for Profile {
        ...
        Self: Send,
    {
        let profile = sqlx::query_as!(
            Profile,
            r#"SELECT id, user_name, stage_name, tag_line, spirit, profile_picture, youtube_profile, twitter_profile, instagram_profile, password_hash, is_claimed, profile_type as "profile_type:ProfileType", color_theme, role_name, current_peak_recommendations, current_peak_library, created_at FROM profiles WHERE id = $1"#,
            resource_id
        )
        .fetch_optional(db)
        .await?
