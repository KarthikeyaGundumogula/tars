...
pub async fn sign_up_artist_handler(
    ...
    let artist = Profile {
        ...
        youtube_profile: data.youtube_profile.map(|p| p.to_string()),
        twitter_profile: data.twitter_profile.map(|p| p.to_string()),
        instagram_profile: data.instagram_profile.map(|p| p.to_string()),
        password_hash,
